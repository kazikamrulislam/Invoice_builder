  ...
  <div>
    <section class="client_section">
      ...
      >
        <b-form
          @submit.prevent="
            buttonClick == 'Create Client' ? createClient() : updateClient()
          "
        >
          <b-row>
          ...
          <b-row class="popup_submit_button" align-h="center">
            <b-button class="mr-3" type="submit" variant="primary">{{buttonClick}}</b-button>
      ...
      <b-row class="top_row">
        <b-col sm="4" md="6" lg="6" xl="6">
          <div class="creat_client_left">
            <h2>Clients</h2>
            <div class="clint_count">
              {{ dashboardStatistics.totalclients }} pepole found
            </div>
          </div>
        </b-col>
        <b-col class="clients_detail_col" sm="8" md="6" lg="6" xl="6">
          <div
            class="creat_client_right">
            <BaseButton
              @onClick="onClickBtn"
              :type="'subbmit'" >
              + Create Client
      ...
      >
        <div class="clients">
          <div class="clients_left">
            <div class="clients_left_content">
              <div class="avatar">
              ...
              <div class="client_adress">
                <div class="client_address">{{ item.address }}</div>
              </div>
            </div>
          </div>
          <div class="clients_right">
            <div class="clients_right_content">
...
export default {
  ...
  methods: {
    onClickBtn(e){
      this.buttonClick = "Create Client";
    ...
    async createClient() {
      try {
        ...
        await this.$axios
          .$post("/createclient", cleintBody)
          .then((response) => {
            self.$bvModal.hide("modal-add-client");
    ...
    async deleteUser(id) {
      let self = this;
      await this.$axios.$get("/deleteclient/" + id).then(function (response) {
        self.getClients();
        self.toast({
                    header: 'Success',
                    body: response.message,
    ...
    async editClient(id) {
      ...
      await this.$axios.$get("/singleclient/" + id).then((response) => {
        self.$bvModal.show("modal-add-client");
        self.buttonClick = "Update Client";
...
.creat_client_left h2{
  font-size: 20px;
  font-weight: 500;
  color: #93B0C8;
}
.creat_client_left .clint_count{
  font-size: 14px;
  line-height: 1.2;
  color: #707EAE;
}
...
/* .creat_client_right button {
  padding: 20px 50px;
  background: #0075ff;
} */

.creat_client_right button:focus {
  box-shadow: none;
}
...
.form-group :deep(input),.form-group :deep(select){
  height: 50px;
}
...
.client_details .client_info h2 {
  font-size: 18px;
  font-weight: 700;
}

.client_details .client_info h3 {
  font-size: 16px;
  font-weight: 700;
}

.client_details .client_info .client_mail,
.client_details .client_info .client_id,
.client_details .client_info .client_address {
  font-size: 15px;
}
